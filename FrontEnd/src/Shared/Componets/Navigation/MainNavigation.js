...
const MainNavigation = (props) => {
    ...
    const handleSideBar = ()=>{
      ...
      const labelConatains =  document.querySelector('.MainHeader_hamburger__2ek3H').classList.toggle('active');
       if (labelConatains) {
  ...
  <SideDrawer show={checked} onClick={closeSideDrawer}>
        ...
        <MainHeader>
            <Container fluid className='d-flex align-items-center px-4 px-md-4 px-lg-5'>
                ...
                <Link to='/'>
                    <div className={styles.headerTitle}>PlacesApp</div>
                </Link>
                <nav className={styles.mainNavigation +" " + 'mx-4 mx-md-4 mx-lg-5'}>
                    <NavLinks/>
                </nav>
            </Container>
        </MainHeader>
